...
import { Wallet, Plus, Moon, Sun, LogOut, Lock, Menu, X, Flame } from 'lucide-react';
          ...
          <>
            ...
            >
              <SidebarBody
                ...
                onLogout={closeDrawerAnd(onLogout)}
                userEmail={userEmail}
...
function SidebarBody({
  ...
  return (
    <>
      ...
      {/* Footer */}
      <div className="p-3 border-t border-slate-200/80 dark:border-white/15 space-y-1">
        {userEmail && (
          <p className="px-3 text-[10px] text-slate-400 dark:text-slate-500 truncate" title={userEmail}>
            {userEmail}
          </p>
        )}
        <div className="flex items-center gap-1">
          ...
          {onLogout && (
            <button
              onClick={onLogout}
              className="inline-flex items-center justify-center p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-white/40 dark:hover:bg-white/10 transition-colors"
              title="Cerrar sesión"
